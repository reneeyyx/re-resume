...
def toggle_filter_category(page, category_name, options_to_select):
    """
    Generic function to find a filter dropdown, open it, toggle specific items, and close it.
    """
    logger.info(f"🌪️ Applying Filter Category: {category_name}")
    
    try:
        # 1. FIND BUTTON (Partial text match is safer)
        # We look for a button with class drop-down__btn containing the category name
        btn = page.locator("button.drop-down__btn").filter(has_text=category_name).first
        
        if not btn.is_visible():
            logger.warning(f"⚠️ Button for '{category_name}' not found. Skipping.")
            return

        logger.info(f"Clicking '{category_name}' button...")
        btn.click()

        # Wait for dropdown to open (check for search input)
        try:
        ...
        for option in options_to_select:
            # Find row containing exact text of the option
            row = page.locator("li.drop-down__item").filter(has_text=option).first
            
            if row.is_visible():
                # Check if "toggle-on" icon is visible
                is_on = row.locator("i.toggle-on").is_visible()
                
                if not is_on:
                    logger.info(f"🔘 '{option}' is OFF. Turning ON...")
                    row.locator("label.toggle--single").click()
                    time.sleep(0.5) # Brief pause for animation
                else:
                    logger.info(f"✅ '{option}' is already ON.")
            else:
                logger.warning(f"⚠️ Option '{option}' not found in list.")

        # 3. CLOSE & RELOAD
        logger.info("🔄 Closing dropdown to trigger table reload...")
        page.locator("body").click(position={"x": 0, "y": 0})
        
        logger.info("⏳ Waiting for table to refresh...")
        time.sleep(5) 
        # Optional: Wait for table to stabilize
        try:
            page.wait_for_selector("table tbody tr", timeout=10000)
        except: pass
        
    except Exception as e:
        logger.error(f"❌ Error in filter category '{category_name}': {e}")
